...
ホストが一括確定処理を行う時だけ、host-only snapshot APIで内部入力を取得する。

更新系APIはRoom参加時に発行されたplayer tokenを必須とし、`playerId` だけでは操作できない。

### 最終シーズン

Season 10は結果確認完了後に `game-complete` へ進み、契約・育成・放出・ドラフト・競売には入らない。
