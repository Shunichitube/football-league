...
- 既存の選手詳細表示は維持する。
- 控え一覧はポジション順／総合ランク順／年齢順／契約年数順に切り替えられる。選択した項目を最優先し、その後はポジション（GK→DF→MF→FW）・ランク・年齢・契約年数・加入順を画面ごとの規定順でタイブレークに使う。ドラフト、育成、放出画面も同じ4種の並び替えを備える。内部Overall数値は表示しない。
...
- ドラフト、競売、育成対象選択、重点育成選択の途中では保存を無効にする。セーブ画面を開いた場合も元画面の保存可否を引き継ぐ。
...
- 能力カードはSS〜Gのランクのみを表示し、内部能力値と `hiddenGrowth` は表示しない。フィールド選手はスタミナも同じランクバーで表示し、GKにはスタミナを表示しない。
- ポジション表記と内部キーは `GK / DF / MF / FW` に統一する。現行セーブversion 2のみを読み込み対象とし、旧セーブのポジション変換互換は維持しない。
- Stage 20訂正: 選手カードは左にプロフィール・適正・伸びやすい能力、右に能力ランクバーと特殊能力を配置する。能力順は走力、パス、ドリブル、シュート、守備、6行目はフィールド選手がスタミナ、GKがGK能力とする。


### 試合イベントログ

- 内部イベントkindは試合計算用に保持し、表示は自然な日本語へ整形する。
- パスは「AからBへパスが通る」のように送り手と受け手を表示する。
- ゴール行のみ黄色・太字で強調し、先頭にその時点のスコアを置く。
- 同点は「○○クラブが同点に追いついた」、逆転は「○○クラブが逆転」、同点から前に出た場合は「○○クラブがリード」と表示する。
- 旧イベント文字列を解析する互換処理は持たず、イベントに保存された構造化表示情報を使用する。
- イベントログの最後には `試合終了　HOMEクラブ　1－2　AWAYクラブ` の形式で最終スコアを通常表示する。
