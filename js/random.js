...
export function createRandom(seed, savedState = null) {
  let state = savedState === null ? seedToInt(seed) || 1 : savedState | 0;
  ...
  return { next, int: (min, max) => Math.floor(next() * (max - min + 1)) + min, pick: xs => xs[Math.floor(next() * xs.length)], snapshot: () => state };
}
