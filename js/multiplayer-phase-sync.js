...
function statusClass(phase, player) {
  return player.phaseComplete || player.ready ? 'mp-ready' : 'mp-not-ready';
}

function renderConfirmPanel(room, localPlayer) {
  if (room.phase !== 'season-result') return '';
  const done = room.players.filter(player => player.phaseComplete).length;
  const incomplete = room.players.filter(player => !player.phaseComplete);
  const localDone = Boolean(localPlayer?.phaseComplete);
  return `<section class="match-card" data-mp-phase-sync-panel>
    ...
    ${localPlayer ? `<button type="button" data-mp-confirm-phase="${escapeHtml(room.roomId)}" ${localDone ? 'disabled' : ''}>${localDone ? '確認完了済み' : '結果確認完了'}</button>` : ''}
  </section>`;
}
