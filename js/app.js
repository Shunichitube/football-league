import { applySeasonFinances, awards, createLeague, finalizeSeason, simulateRemainingSeason, standings, startNextSeason } from './league.js?v=0.14.0';
import { STAT_LABELS } from './data.js';
import { createRandom } from './random.js';
import { createAuctionPool, createDraftPool, resolveAuctionActions, resolveDraftActions } from './market.js?v=0.14.0';
import { escapeHtml as e, renderLineupEditor, renderMatchDetail, renderPlayerCard, renderPlayerDetail, renderRosterPanel, renderSeasonMatchList, renderSeasonPlayerStats } from './ui.js?v=0.14.0';
import { decideCpuAuctionAction, decideCpuDraftAction, prepareCpuClubs, prepareCpuMarketSpace, processLeagueOffseason, selectBestLineup } from './cpu.js?v=0.13.1';
...
function title(){return `<main class="title"><p>5人制クラブ運営ゲーム</p><h1>FOOTBALL<br><b>LEAGUE</b></h1><button data-a="setup">新しく始める</button><button data-stage19="loadTitle" class="subtle">続きから</button><footer>v0.14.1・Stage 19</footer></main>`}
...
function squad(){let c=me(),lineup=validateLineup(c);return `${head()}<main style="--club:${e(c.color)}"><h2>編成</h2><p class="hint">所属選手を選び、配置したい枠を押してください。能力はすべてランク表示です。</p>${renderLineupEditor(c,s.selectedLineupPlayerId,s.lineupMessage,s.lineupError,s.benchSort)}<button data-a="season" ${lineup.ok?'':'disabled'}>シーズンをシミュレート</button><button data-nav="home" class="subtle">戻る</button></main>`}
...
function startAuction(){const season=s.league.season;s.auction={pool:createAuctionPool(s.league.seed,season,s.league.releasedPlayers),i:0,rng:createRandom(`${s.league.seed}:season:${season}:auction`)};s.view='auction';s.note=`シーズン${season}ドラフト終了。競売を開始します。`}
...
document.addEventListener('change', event => { if (event.target.matches('[data-bench-sort]')) { s.benchSort=event.target.value; render(); } });
