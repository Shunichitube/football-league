import { applySeasonFinances, awards, createLeague, finalizeSeason, simulateRemainingSeason, standings, startNextSeason } from './league.js?v=0.17.3';
...
import { escapeHtml as e, renderLineupEditor, renderMatchDetail, renderPlayerCard, renderRosterPanel, renderSeasonMatchList, renderSeasonPlayerStats } from './ui.js?v=0.17.3';
...
function stage5Render() {
  ...
  if (s.view === 'squad') {
    ...
    app.querySelector('.lineup-editor')?.insertAdjacentHTML('afterbegin', `<section class="match-card tactic-panel"><p class="eyebrow">戦術</p><div class="tactic-row">${Object.keys(labels).map(t => `<button data-tactic="${t}" class="${c.tactic===t ? '' : 'subtle'}">${labels[t]}</button>`).join('')}</div><p class="hint">現在：${labels[c.tactic]} — ${descriptions[c.tactic]}</p></section>`);
  }
}
render = stage5Render;
