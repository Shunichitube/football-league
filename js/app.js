import { applySeasonFinances, awards, createLeague, playCurrentRound, standings, startNextSeason } from './league.js';
...
function draftEligibleIds(){return s.league.clubs.filter(club=>club.funds>=1&&club.roster.length<12&&(!s.draft?.humanDeclined||club.id!==s.league.humanClubId)).map(club=>club.id)}
...
function runCpuDraft(){let guard=0;while(s.view==='draft'&&guard++<100){let d=s.draft;if(d.pendingClubIds.includes(s.league.humanClubId))break;if(!d.pendingClubIds.length){if(advanceDraftRound())break;continue}const result=resolveSimultaneousDraftCycle({clubs:s.league.clubs,candidates:d.pool,pendingClubIds:d.pendingClubIds,humanClubId:s.league.humanClubId,rng:d.rng});d.pool=result.candidates;d.pendingClubIds=result.pendingClubIds;if(!result.acquired.length&&result.declinedIds.length===0){d.pendingClubIds=[]}}}
function beginDraft(){const season=s.league.season;prepareCpuMarketSpace(s.league);s.draft={pool:createDraftPool(s.league.seed,season),round:1,rng:createRandom(`${s.league.seed}:season:${season}:draft`),pendingClubIds:s.league.clubs.filter(club=>club.funds>=1&&club.roster.length<12).map(club=>club.id),humanDeclined:false};s.view='draft';s.note=`シーズン${season}ドラフトを開始します。`;runCpuDraft()}
function pickDraft(p){let d=s.draft;if(!d.pendingClubIds.includes(s.league.humanClubId))return;const result=resolveSimultaneousDraftCycle({clubs:s.league.clubs,candidates:d.pool,pendingClubIds:d.pendingClubIds,humanClubId:s.league.humanClubId,humanPickId:p.id,rng:d.rng});d.pool=result.candidates;d.pendingClubIds=result.pendingClubIds;const acquired=result.acquired.find(row=>row.clubId===s.league.humanClubId);if(!acquired){s.note=`${p.name}は競合抽選で外れました。外れクラブとして再指名してください。`;return}s.note=`${p.name}を${acquired.contested?'競合抽選で':''}獲得しました。`;runCpuDraft()}
function bid(amount){let a=s.auction,p=a.pool[a.i],bids=s.league.clubs.map(c=>({c,n:c.id===1?(c.roster.length<12?amount:0):cpuBid(c,p,a.rng)})),high=Math.max(...bids.map(x=>x.n));if(high>0){let top=bids.filter(x=>x.n===high),w=top[a.rng.int(0,top.length-1)];addPlayer(w.c,p,high);s.note=`落札 — ${w.c.name} が ${high}ptで獲得しました。`}else s.note=`${p.name}は見送りになりました。`;a.i++;if(a.i>=a.pool.length)prepareCpuClubs(s.league)}
app.addEventListener('click',ev=>{let a=ev.target.closest('[data-a]')?.dataset.a,n=ev.target.closest('[data-nav]')?.dataset.nav,p=ev.target.closest('[data-p]')?.dataset.p;if(n){s.view=n;return render()}if(p){let x=s.draft.pool.find(q=>q.id===p);if(x&&me().funds>=1&&me().roster.length<12)pickDraft(x);return render()}if(a==='setup'||a==='title'){s.view=a;return render()}if(a==='start'){let name=document.querySelector('#name').value.trim();if(!name)return alert('クラブ名を入力してください。');let seed=document.querySelector('#seed').value.trim()||String(Date.now());s.league=createLeague({name,color:document.querySelector('#color').value,seed});s.offseasonComplete=false;beginDraft();return render()}if(a==='skipDraft'){s.draft.humanDeclined=true;s.draft.pendingClubIds=s.draft.pendingClubIds.filter(id=>id!==s.league.humanClubId);runCpuDraft();return render()}if(a==='bid'||a==='pass'){let n=a==='pass'?0:Number(document.querySelector('#bid').value);if(n<0||n>me().funds)return alert('入札額を確認してください。');bid(n);return render()}if(a==='squad'){s.view='squad';return render()}if(a==='match'){let r=playCurrentRound(s.league);s.round=r.round;s.match=r.userMatch;s.view='result';return render()}if(a==='continue'){s.view='home';return render()}});render();
...
function stage4Render() {
  if (s.view === 'development') {
    const c = me();
    const finance=s.financeSummary?.find(row=>row.clubId===s.league.humanClubId);
