import { applySeasonFinances, awards, createLeague, playCurrentRound, standings, startNextSeason } from './league.js?v=0.9.0';
...
import { escapeHtml as e, renderLineupEditor, renderPlayerCard, renderPlayerDetail, renderRosterPanel } from './ui.js?v=0.9.0';
...
const app=document.querySelector('#app');let s={view:'title',league:null,draft:null,auction:null,match:null,round:0,note:'',rosterOpen:false,detailPlayerId:null,selectedLineupPlayerId:null,lineupMessage:'',lineupError:false};
...
function title(){return `<main class="title"><p>5人制クラブ運営ゲーム</p><h1>FOOTBALL<br><b>LEAGUE</b></h1><button data-a="setup">新しく始める</button><footer>v0.9.0・Stage 14</footer></main>`}
...
function fixture(){let humanId=me().id,f=s.league.schedule[s.league.currentRound-1]?.fixtures.find(f=>f.homeId===humanId||f.awayId===humanId);return f&&{home:s.league.clubs.find(c=>c.id===f.homeId),away:s.league.clubs.find(c=>c.id===f.awayId)}}
function home(){let c=me(),f=fixture(),lineup=validateLineup(c);if(s.league.completed)return `${head()}<main><section class="hero"><h2>シーズン${s.league.season}終了</h2><button data-nav="table">最終順位を見る</button><button data-nav="stats" class="subtle">個人成績を見る</button></section></main>`;return `${head()}<main><section class="hero" style="--club:${c.color}"><p>第${s.league.currentRound}/10節</p><h2>${e(f.home.name)} VS ${e(f.away.name)}</h2>${lineup.ok?'<button data-a="match">この節を進める</button>':`<p class="lineup-error">${e(lineup.error)}</p><button data-nav="squad">編成を確認する</button>`}</section><nav><button data-nav="squad">編成</button><button data-nav="table">順位表</button><button data-nav="stats">個人成績</button></nav></main>`}
function squad(){let c=me(),lineup=validateLineup(c);return `${head()}<main><h2>編成</h2><p class="hint">所属選手を選び、配置したい枠を押してください。能力はすべてランク表示です。</p>${renderLineupEditor(c,s.selectedLineupPlayerId,s.lineupMessage,s.lineupError)}<button data-a="match" ${lineup.ok?'':'disabled'}>この節を進める</button><button data-nav="home" class="subtle">戻る</button></main>`}
...
app.addEventListener('click',ev=>{let a=ev.target.closest('[data-a]')?.dataset.a,n=ev.target.closest('[data-nav]')?.dataset.nav,p=ev.target.closest('[data-p]')?.dataset.p;if(n){s.view=n;if(n!=='squad'){s.selectedLineupPlayerId=null;s.lineupMessage='';s.lineupError=false}return render()}if(p){let x=s.draft.pool.find(q=>q.id===p);if(x&&me().funds>=1&&me().roster.length<12)pickDraft(x);return render()}if(a==='setup'||a==='title'){s.view=a;return render()}if(a==='start'){let name=document.querySelector('#name').value.trim();if(!name)return alert('クラブ名を入力してください。');let seed=document.querySelector('#seed').value.trim()||String(Date.now());s.league=createLeague({name,color:document.querySelector('#color').value,seed});s.offseasonComplete=false;beginDraft();return render()}if(a==='skipDraft'){s.draft.humanDeclined=true;s.draft.pendingClubIds=s.draft.pendingClubIds.filter(id=>id!==me().id);runCpuDraft();return render()}if(a==='bid'||a==='pass'){let n=a==='pass'?0:Number(document.querySelector('#bid').value);if(n<0||n>me().funds)return alert('入札額を確認してください。');bid(n);return render()}if(a==='squad'){s.view='squad';return render()}if(a==='match'){let lineup=validateLineup(me());if(!lineup.ok)return alert(lineup.error);let r=playCurrentRound(s.league);s.round=r.round;s.match=r.userMatch;s.view='result';return render()}if(a==='continue'){s.view='home';return render()}});render();
